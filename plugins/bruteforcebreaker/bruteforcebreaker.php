...
*
* @version 1.3 CRtEuropeModified Version
* @author CRtEurope https://www.cesidianroot.eu
* @url https://github.com/CRtEurope/roundcubewebmail/tree/master/plugins/
* @license GPLv3
*/
class bruteforcebreaker extends rcube_plugin {
    ...
    function ban_loginFailed($args) {
        ...
        if ($this->rc->config->get('bruteforcebreaker_keep_trace', true)) 
            rcube::write_log('bruteforcebreaker', sprintf("Login failed for %s. Number of attemps: %d.\n", $ip, $this->data_ban['FAILURES'][$ip]));
        ...
        if ($this->data_ban['FAILURES'][$ip] > ($this->rc->config->get('bruteforcebreaker_nb_attemps', 5) -1 )) {
            ...
            if ($this->rc->config->get('bruteforcebreaker_keep_trace', true)) 
                rcube::write_log('bruteforcebreaker', sprintf("IP address banned from login - too many attemps (%s)\n", $ip));
        }
